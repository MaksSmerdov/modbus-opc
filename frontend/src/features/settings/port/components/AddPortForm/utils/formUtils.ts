...
export function getDefaultPortFormValues(initialData?: CreatePortData): Partial<PortFormData> {
  if (initialData) {
    ...
    } else if (initialData.connectionType === 'TCP') {
      return {
        connectionType: 'TCP',
        name: initialData.name,
...
/**
 * Получает значения для сброса формы при смене типа подключения на TCP_RTU
 */
export function getTCPRTUResetValues(): Partial<PortFormData> {
  return {
    connectionType: 'TCP_RTU' as const,
    isActive: true,
  };
}
...
export function getTCPFieldError(
  field: keyof (TCPFormData & TCPRTUFormData),
  connectionType: string,
  errors: FieldErrors<PortFormData>
): TCPFieldErrors[keyof TCPFormData] | TCPRTUFieldErrors[keyof TCPRTUFormData] | undefined {
  if (connectionType === 'TCP') {
    const tcpErrors = errors as TCPFieldErrors;
    return tcpErrors[field as keyof TCPFormData];
  }
  if (connectionType === 'TCP_RTU') {
    const tcpRtuErrors = errors as TCPRTUFieldErrors;
    return tcpRtuErrors[field as keyof TCPRTUFormData];
  }
  return undefined;
}
