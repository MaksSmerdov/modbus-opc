...
export const PortsList = memo(({ onEdit }: PortsListProps) => {
  ...
  const {
    deleteConfirmOpen,
    isDeleting,
    devicesCount: deletingPortDevicesCount,
    handleDeleteClick,
    handleDeleteConfirm,
    handleDeleteCancel,
  } = usePortDeletion();
  ...
  const devicesCountByPort = useMemo(() => countDevicesByPort(ports, devices), [devices, ports]);
  ...
  const handleDeletePort = useCallback(
    (portId: string, devicesCount: number) => {
      if (canManagePorts) {
        handleDeleteClick(portId, devicesCount);
      }
    },
  ...
  const deleteMessage = useMemo(() => {
    if (deletingPortDevicesCount > 0) {
      const devicesText = formatDevicesCount(deletingPortDevicesCount);
      return `На этом порту ${
        devicesText ? devicesText.replace(/[\[\]]/g, '') : `${deletingPortDevicesCount} устройств`
      }. Вы уверены, что хотите удалить этот порт? Это действие нельзя отменить.`;
    }
    return 'Вы уверены, что хотите удалить этот порт? Это действие нельзя отменить.';
  }, [deletingPortDevicesCount]);
  ...
  if (error) {
    return (
      <div className={styles['portsList']}>
        <div className={styles['portsList__error']}>Ошибка загрузки портов</div>
      </div>
    );
  }
  ...
  if (!ports || ports.length === 0) {
    return (
      <div className={styles['portsList']}>
        <div className={styles['portsList__empty']}>Нет портов</div>
      </div>
    );
  }
